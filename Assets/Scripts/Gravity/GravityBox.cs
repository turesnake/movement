...
public class GravityBox : GravitySource 
{
    ...
    {
		// cube中心 -> 当前点, in os
        position = transform.InverseTransformDirection(position - transform.position);//ws->os

		Vector3 vector = Vector3.zero;

		int outside = 0; // 统计 目标点 在几个轴上是 out 的 [0,3]
		if (position.x > boundaryDistance.x) {
			vector.x = boundaryDistance.x - position.x; // 负
			outside = 1;
		}
		else if (position.x < -boundaryDistance.x) {
			vector.x = -boundaryDistance.x - position.x;// 正
			outside = 1;
		}

		if (position.y > boundaryDistance.y) {
			vector.y = boundaryDistance.y - position.y;// 负
			outside += 1;
		}
		else if (position.y < -boundaryDistance.y) {
			vector.y = -boundaryDistance.y - position.y;// 正
			outside += 1;
		}

		if (position.z > boundaryDistance.z) {
			vector.z = boundaryDistance.z - position.z;// 负
			outside += 1;
		}
		else if (position.z < -boundaryDistance.z) {
			vector.z = -boundaryDistance.z - position.z;// 正
			outside += 1;
		}

		// 若在某个轴上, 目标点非 out, vector 对应的分量为 0
		// 这意味着 vector 要么在 3个轴上, 要么在 3个轴面上
		...
		// 只设置 os 三轴中, 最近的那个轴 的值, 剩余两个分量值 维持0
		// 这样, 当前点只会受到 cube 上一个面的 重力影响
        ...
        return transform.TransformDirection(vector);// os->ws
	}
